...
def _prepare_synthetic_repo(tmp_path: Path, actual_repo_root: Path) -> tuple[Path, Path, Path, Path]:
    ...
    _write_text(
        repo_root / "configs" / "models" / "asvdpp.yaml",
...
def test_run_asvdpp_experiment_writes_valid_run_artifacts(
    tmp_path: Path,
    monkeypatch,
) -> None:
    ...
    monkeypatch.setattr(
        "recsys_lab.experiments.asvdpp.git_snapshot",
    ...
    payload = run_asvdpp_experiment(
        ...
        model_seed=4,
        repo_root=repo_root,
        command="recsys-lab train-asvdpp --synthetic",
    )
    ...
    assert manifest["model"]["name"] == "asvdpp"
    assert metrics["metrics"]["validation_rmse"] >= 0.0
    assert metrics["model"]["split_cache"]["enabled"] is False
