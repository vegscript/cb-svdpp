...
def test_run_random_multiseed_benchmark_aggregates_split_seed_runs(
    ...
    monkeypatch.setattr(
        ...
        lambda _repo_root: {"commit": "b91d606ae78b9170569ccf76683a92ad5d1d2900", "branch": "main", "dirty": False},
    )
    ...
    assert summary["aggregate"]["validation_rmse"]["mean"] == 0.857
    assert summary["aggregate"]["test_rmse"]["mean"] == 0.859
    assert summary["aggregate"]["training_wall_clock_seconds"]["mean"] == 360.0
    assert summary["aggregate"]["peak_memory_mb"]["mean"] == 1410.0
...
def test_run_random_multiseed_benchmark_rejects_mixed_git_state(
    ...
    with pytest.raises(ValueError, match="share identical git commit, branch, and dirty state"):
        run_random_multiseed_benchmark(
            ...
            split_seeds=[1, 2, 3],
            model_seed=1,
            run_manifest_paths=[run_manifest_path_seed_1, run_manifest_path_seed_2, run_manifest_path_seed_3],
            repo_root=repo_root,
            command="recsys-lab benchmark-random-multiseed --synthetic --run-manifest-paths explicit",
        )
