...
def test_materialize_interaction_array_artifacts_from_manifest_writes_expected_arrays(tmp_path: Path) -> None:
    interactions_path = tmp_path / "toy_interactions.parquet"
    pq.write_table(
        pa.table(
            {
...
def test_ratings_data_subset_composes_row_selection_without_copying_base_arrays(tmp_path: Path) -> None:
    interactions_path = tmp_path / "toy_interactions.parquet"
    pq.write_table(
        pa.table(
            {
