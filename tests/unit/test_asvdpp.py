...
import recsys_lab.models.asvdpp as asvdpp_module
...
def test_asvdpp_numba_kernel_matches_python_reference(monkeypatch: MonkeyPatch) -> None:
    data = RatingsData(
    ...
    numba_model = ASVDppRecommender(config).fit(data)

    def fail_kernel(*args: object, **kwargs: object) -> None:
        raise RuntimeError("force python reference")

    monkeypatch.setattr(asvdpp_module, "train_asvdpp_epoch_numba", fail_kernel)
    python_model = ASVDppRecommender(config).fit(data)
