...
class AsymmetricSVDRecommender:
    def __init__(self, config: AsymmetricSVDConfig) -> None:
        self.config = config
    ...
    def _parameter_dtype(self) -> np.dtype:
        if self.config.dtype not in {"float32", "float64"}:
            raise ValueError("asymmetric_svd dtype must be 'float32' or 'float64'")
        ...
        for idx in order:
            ...
            if explicit_items.size > 0:
                residual_weights = explicit_ratings - (
                    self.global_mean + user_bias_old + self.item_bias[explicit_items]
                )
            ...
            user_context = explicit_sum + implicit_sum
            prediction = float(self.global_mean + user_bias_old + item_bias_old + np.dot(item_vector_old, user_context))
            ...
            if explicit_items.size > 0:
                ...
                explicit_update = (
                    self.config.learning_rate * error * explicit_norm * residual_weights[:, None] * item_vector_old
                )
                self.explicit_factors[explicit_items] = (
                    explicit_old + explicit_update - (self.config.learning_rate * self.config.lambda_x * explicit_old)
                )

            if implicit_items.size > 0:
                implicit_old = self.implicit_factors[implicit_items].copy()
                implicit_update = self.config.learning_rate * error * implicit_norm * item_vector_old
    ...
    def _user_context(self, user_id: int) -> np.ndarray:
        ...
        context = np.zeros(self.explicit_factors.shape[1], dtype=np.float64)
    ...
    def predict(self, user_id: int, item_id: int, *, clip: bool = True) -> float:
        if not self.is_fitted:
            raise RuntimeError("asymmetric_svd is not fitted")
    ...
    def predict_many(
        ...
        *,
        ...
        if not self.is_fitted:
            raise RuntimeError("asymmetric_svd is not fitted")
        if self.item_factors is None or self.user_bias is None or self.item_bias is None:
            raise RuntimeError("model parameters are not initialized")

        users = np.asarray(user_ids, dtype=np.int64)
        items = np.asarray(item_ids, dtype=np.int64)
        contexts = np.zeros((self.user_bias.shape[0], self.item_factors.shape[1]), dtype=np.float64)
