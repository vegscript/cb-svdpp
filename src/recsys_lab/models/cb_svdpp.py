...
class CBSVDppRecommender:
        ...
        *,
        ...
        for idx in order:
            ...
            if history_items.size > 0:
                context += implicit_norm * one_minus_alpha * np.sum(self.implicit_factors[history_items], axis=0)
            ...
            if history_items.size > 0:
                ...
                self.implicit_factors[history_items] = (
            ...
            if history_clusters.size > 0:
                ...
                self.implicit_cluster_factors[history_clusters] = (
    ...
    def fit(
        ...
        *,
        ...
        user_cluster_histories: UserClusterCountIndex | None = None,
    ) -> "CBSVDppRecommender":
    ...
    def predict_many(
        ...
        *,
        ...
        mixed_item_factors = self._ensure_mixed_item_factors_cache()[items]

        predictions = (
            self.global_mean
            + self.user_bias[users].astype(np.float64, copy=False)
            + self.item_bias[items].astype(np.float64, copy=False)
            + np.sum(contexts * mixed_item_factors, axis=1)
        )
        if clip:
