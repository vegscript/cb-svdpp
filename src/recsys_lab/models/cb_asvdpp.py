        ...
        *,
        ...
        for idx in order:
            ...
            if explicit_items.size > 0:
                ...
                residual_weights = explicit_ratings - (self.global_mean + user_bias_old + explicit_item_biases)
                ...
                context += (
                    ...
                    * alpha
                    * np.sum(
                        residual_weights[:, None] * self.explicit_cluster_factors[explicit_clusters],
                        axis=0,
                    )
                )
            ...
            prediction = float(self.global_mean + user_bias_old + item_bias_old + np.dot(q_mix_old, context))
            ...
            self.user_cluster_factors[user_cluster_id] = user_cluster_vector_old + (
                ...
                * (error * alpha * q_mix_old - self.config.lambda_pC * user_cluster_vector_old)
            )
            ...
            self.item_cluster_factors[item_cluster_id] = item_cluster_vector_old + (
                self.config.learning_rate * (error * alpha * context - self.config.lambda_qC * item_cluster_vector_old)
            )
            ...
            if explicit_items.size > 0:
                ...
                for history_pos, history_cluster in enumerate(explicit_clusters):
                    ...
                    self.explicit_cluster_factors[cluster_id] = (
                        cluster_old + cluster_update - (self.config.learning_rate * self.config.lambda_xC * cluster_old)
                    )

            if implicit_items.size > 0:
                implicit_old = self.implicit_factors[implicit_items].copy()
                implicit_update = self.config.learning_rate * error * implicit_norm * one_minus_alpha * q_mix_old
                self.implicit_factors[implicit_items] = (
            ...
            if implicit_clusters.size > 0:
                ...
                self.implicit_cluster_factors[implicit_clusters] = (
