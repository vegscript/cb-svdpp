...
def run_ml100k_paper_benchmark(
    *,
    ...
    if (
        runtime_config_path.suffix != ".yaml"
        or device_config_path.suffix != ".yaml"
        or model_config_path.suffix != ".yaml"
    ):
    ...
    with runtime_execution_context(threading_config=threading_config):
        ...
        try:
            ...
            training_time = aggregate["training_wall_clock_seconds"]
            markdown_lines.extend(
                [
                    "",
                    "## Aggregate",
                    "",
                    f"- test_rmse count: `{aggregate['test_rmse']['count']}`",
                    f"- test_rmse mean: `{aggregate['test_rmse']['mean']:.6f}`",
                    f"- test_rmse std: `{aggregate['test_rmse']['std']:.6f}`",
                    f"- test_rmse median: `{aggregate['test_rmse']['median']:.6f}`",
                    f"- train_rmse mean: `{aggregate['train_rmse']['mean']:.6f}`",
                    (f"- training_wall_clock_seconds cv: `{training_time['coefficient_of_variation']:.6f}`"),
                    f"- training_wall_clock_seconds mean: `{training_time['mean']:.2f}`",
                ]
            )
            ...
            failed_manifest = {
                **benchmark_manifest,
                ...
                "inputs": {
                    "run_ids": [
                        str(_read_run_manifest(path)["run_id"]) for path in run_manifest_paths if path.exists()
                    ],
                    "run_manifest_paths": [
