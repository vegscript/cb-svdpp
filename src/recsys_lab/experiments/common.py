...
from recsys_lab.experiments.runtime import assess_device_profile_contract, resolve_runtime_threading_config
...
def offset_utc_timestamp(timestamp: str, *, seconds: int) -> str:
    if seconds < 0:
        raise ValueError("seconds must be >= 0")
    shifted = datetime.strptime(timestamp, "%Y-%m-%dT%H%M%SZ").replace(tzinfo=timezone.utc) + timedelta(seconds=seconds)
    return shifted.strftime("%Y-%m-%dT%H%M%SZ")


def reserve_timestamped_artifact_dir(
    *,
    artifacts_root: Path,
    id_from_timestamp: Callable[[str], str],
) -> tuple[str, str, Path]:
    base_timestamp = utc_timestamp()
    collision_offset_seconds = 0

    while True:
        timestamp = offset_utc_timestamp(base_timestamp, seconds=collision_offset_seconds)
        artifact_id = id_from_timestamp(timestamp)
        artifact_dir = artifacts_root / artifact_id
        try:
            artifact_dir.mkdir(parents=True, exist_ok=False)
        except FileExistsError:
            collision_offset_seconds += 1
            continue
        return timestamp, artifact_id, artifact_dir
...
def split_id(split_family: str, split_config: SplitConfig) -> str:
    ...
    return f"{split_family}_tr{train_pct:03d}_va{validation_pct:03d}_{seed_slug(split_config.seed)}"
...
def paper_faithful_ml100k_inner_split_id(
    *,
    ...
    return f"paper_faithful_ml100k_inner_v1_u{fold_index}_va{validation_pct:03d}_{seed_slug(inner_seed)}"
...
def build_runtime_metadata(
    *,
    ...
    runtime: dict[str, Any] = {
        ...
        "dtype": runtime_dtype,
        "device_profile_contract": assess_device_profile_contract(device_config_payload=device_config_payload),
