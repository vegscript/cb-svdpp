    ...
    *,
    ...
    measurement = build_benchmark_measurement(
        ...
        notes=[
            ...
            "The memmap variant compares processed manifest loading with interaction array sidecars against parquet-only loading.",
        ],
    )
        ...
        for scenario_name, scenario_payload in summary_payload["scenarios"].items():
            ...
            markdown_lines.extend(
                [
                    ...
                    "| --- | ---: | ---: | ---: |",
                    f"| `reference_parquet` | {reference['data_load_wall_clock_seconds']['mean']:.6f} | {reference['data_load_wall_clock_seconds']['coefficient_of_variation']:.6f} | {reference['rows_per_second']['mean']:.6f} |",
                    f"| `optimized_memmap_arrays` | {optimized['data_load_wall_clock_seconds']['mean']:.6f} | {optimized['data_load_wall_clock_seconds']['coefficient_of_variation']:.6f} | {optimized['rows_per_second']['mean']:.6f} |",
                    "",
                    f"- time_speedup_reference_over_optimized: `{scenario_payload['comparison']['data_load_wall_clock_seconds_speedup_reference_over_optimized']:.6f}`",
                    f"- throughput_speedup_optimized_over_reference: `{scenario_payload['comparison']['rows_per_second_speedup_optimized_over_reference']:.6f}`",
                    "",
                ]
            )
