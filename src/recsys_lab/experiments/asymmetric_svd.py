...
def _build_asymmetric_svd_config(
    *,
    ...
    return AsymmetricSVDConfig(
        ...
        lambda_b=float(training.get("lambda_b", 0.04)),
        lambda_q=float(training.get("lambda_q", 0.04)),
        lambda_x=float(training.get("lambda_x", 0.04)),
        lambda_y=float(training.get("lambda_y", 0.04)),
        seed=model_seed,
...
def run_asymmetric_svd_experiment(
    *,
    ...
    command_string = command or (
        "recsys-lab train-asymmetric-svd "
    ...
    with runtime_execution_context(threading_config=threading_config):
        ...
        try:
            ...
            system_metrics = build_system_metrics(
                ...
                inference_rows=len(split.train) + len(split.validation) + len(split.test),
            ...
            write_log(
                ...
                [
                    ...
                    (
                        "explicit_summary "
                        f"users_with_explicit_history={metrics_payload['model']['explicit_summary']['users_with_explicit_history']} "
                        f"mean_explicit_history_size={metrics_payload['model']['explicit_summary']['mean_explicit_history_size']:.4f} "
                        f"max_explicit_history_size={metrics_payload['model']['explicit_summary']['max_explicit_history_size']}"
                    ),
