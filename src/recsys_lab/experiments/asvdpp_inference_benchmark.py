...
def _predict_many_unique_context_reference(
    ...
    *,
    ...
    if model.item_factors is None or model.user_bias is None or model.item_bias is None or model.user_factors is None:
...
def run_asvdpp_inference_cache_benchmark(
    *,
    ...
    benchmark_scope = f"development_ml100k_asvdpp_inference_cache_compare_s{split_seed:03d}_epochs{reduced_epochs}"
    ...
    measurement = build_benchmark_measurement(
        ...
        notes=[
            ...
            (
                "Reference recomputes unique-user contexts on each call; "
                "optimized uses a lazy persistent user-context cache."
            ),
        ],
    )
