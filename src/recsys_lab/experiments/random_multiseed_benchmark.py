...
def _validate_selected_run(
    *,
    ...
    if _processed_manifest_contract(loaded_processed_manifest) != processed_manifest_contract:
        raise ValueError(
            "processed manifest contract mismatch for "
            f"split_seed={split_seed}; canonical dataset contract differs"
        )
...
def run_random_multiseed_benchmark(
    *,
    ...
    command_string = command or (
        ...
        + (
            f" --run-manifest-paths {','.join(run_manifest_path_refs)}"
            if run_manifest_path_refs is not None
            else ""
        )
    )
        ...
        try:
            ...
            markdown_lines.extend(
                [
                    ...
                    f"- training_wall_clock_seconds cv: `{aggregate['training_wall_clock_seconds']['coefficient_of_variation']:.6f}`",
                ]
            )
