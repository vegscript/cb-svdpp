...
def _predict_many_unique_context_reference(
    ...
    *,
    ...
    mixed_item_factors = (
        one_minus_alpha * model.item_factors[items].astype(np.float64, copy=False)
        + alpha
        * model.item_cluster_factors[model.item_clusters[items]].astype(np.float64, copy=False)
    )
    predictions = (
        model.global_mean
...
def run_cb_svdpp_inference_cache_benchmark(
    *,
    ...
    benchmark_scope = (
        f"development_ml100k_cb_svdpp_inference_cache_compare_s{split_seed:03d}_epochs{reduced_epochs}"
    )
    ...
    measurement = build_benchmark_measurement(
        ...
        notes=[
            ...
            "Reference recomputes unique-user contexts and per-query mixed item factors on each call; optimized uses lazy persistent caches.",
        ],
    )
        ...
        for scenario_name, scenario_payload in summary_payload["scenarios"].items():
            ...
            markdown_lines.extend(
                [
                    ...
                    f"- reference_mixed_item_rows_materialized_per_call: `{scenario_payload['materialization']['reference_mixed_item_rows_materialized_per_call']}`",
                    f"- optimized_persistent_user_context_rows: `{scenario_payload['materialization']['optimized_persistent_user_context_rows']}`",
                    f"- optimized_persistent_mixed_item_rows: `{scenario_payload['materialization']['optimized_persistent_mixed_item_rows']}`",
                    ...
                    f"- optimized_mixed_item_rows_materialized_per_call: `{scenario_payload['materialization']['optimized_mixed_item_rows_materialized_per_call']}`",
