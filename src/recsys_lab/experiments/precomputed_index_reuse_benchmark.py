            ...
            for model_name in model_names:
                ...
                for variant_name, reuse_precomputed_indices in (
                    ...
                    for _ in range(measured_repeats):
                        ...
                        train_throughput_samples.append(
                            float(metrics["system_metrics"]["ratings_per_second_train"])
                        )
                    ...
                    variant_measurements[variant_name] = {
                        "warmup": {
                            ...
                            "precomputed_index_reuse": bool(
                                warmup_metrics["model"]["precomputed_index_reuse"]
                            ),
                        },
                ...
                optimized_throughput = variant_measurements[
                    "optimized_reuse_precomputed_indices"
                ]["summary"]["aggregate"]["ratings_per_second_train"]["mean"]
                ...
                model_summary["comparison"] = {
                    ...
                    "ratings_per_second_train_speedup_optimized_over_reference": (
                        float(optimized_throughput) / float(reference_throughput)
        ...
        for model_name, model_summary in summary_payload["models"].items():
            ...
            markdown_lines.extend(
                [
                    ...
                    "| --- | ---: | ---: | ---: |",
                    f"| `reference_rebuild_indices` | {reference['train_time_total']['mean']:.6f} | {reference['train_time_total']['coefficient_of_variation']:.6f} | {reference['ratings_per_second_train']['mean']:.6f} |",
                    f"| `optimized_reuse_precomputed_indices` | {optimized['train_time_total']['mean']:.6f} | {optimized['train_time_total']['coefficient_of_variation']:.6f} | {optimized['ratings_per_second_train']['mean']:.6f} |",
                    "",
                    f"- train_time_speedup_reference_over_optimized: `{model_summary['comparison']['train_time_total_speedup_reference_over_optimized']:.6f}`",
                    f"- throughput_speedup_optimized_over_reference: `{model_summary['comparison']['ratings_per_second_train_speedup_optimized_over_reference']:.6f}`",
                    "",
                ]
            )
