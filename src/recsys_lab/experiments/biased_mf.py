    ...
    with runtime_execution_context(threading_config=threading_config):
        ...
        try:
            ...
            if requested_split_family == "benchmark_random_v1":
                ...
                raise ValueError(f"unsupported split family for biased_mf: {requested_split_family}")

            model = BiasedMFRecommender(
                _build_biased_mf_config(
                    model_config_payload=model_config_payload,
                    model_seed=model_seed,
                    runtime_dtype=runtime_dtype,
                )
            ...
            with PeakMemoryMonitor() as memory_monitor:
                ...
                validation_predictions = None if split.validation is None else model.predict_dataset(split.validation)
