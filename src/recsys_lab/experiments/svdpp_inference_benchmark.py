...
from recsys_lab.data.splitters import official_ml100k_paper_faithful_split
...
from recsys_lab.experiments.svdpp import _build_svdpp_config, run_svdpp_experiment
from recsys_lab.models.svdpp import SVDppRecommender
...
def _predict_many_full_context_reference(
    ...
    *,
    ...
    contexts = np.zeros((model.user_factors.shape[0], model.user_factors.shape[1]), dtype=np.float64)
    contexts[:] = model.user_factors.astype(np.float64, copy=False)
    ...
    for user_id in np.unique(users):
        ...
        contexts[int(user_id)] += norm * np.sum(model.implicit_factors[history], axis=0).astype(np.float64, copy=False)
...
def run_svdpp_inference_materialization_benchmark(
    *,
    ...
    if str(processed_manifest["dataset_short_name"]) != "ml100k":
        raise ValueError("svdpp inference materialization benchmark requires dataset_short_name='ml100k'")
    ...
    benchmark_id = "_".join(
        [
            ...
            "ml100k",
            "svdpp",
            "inference_materialization_compare",
            f"epochs{reduced_epochs}",
            device_profile_name,
        ]
    )
    benchmark_scope = f"development_ml100k_svdpp_inference_materialization_compare_u{fold_index}_epochs{reduced_epochs}"
    ...
    command_string = command or (
        "development benchmark svdpp inference materialization compare "
        ...
        f"--model-seed {model_seed} --fold-index {fold_index} --epochs {reduced_epochs}"
    )
    measurement = build_benchmark_measurement(
        time_metric="inference_wall_clock_seconds",
        time_metric_semantics=(
            "Per-variant inference time on a single trained svdpp model. "
        ...
        notes=[
            ...
            "Both inference variants use the same trained model state; only context materialization differs.",
        ],
    )
    ...
    benchmark_model_config.setdefault("training", {})["training_backend"] = "numba"
    benchmark_model_config_path = config_dir / "svdpp_inference_benchmark.yaml"
    ...
    try:
        training_payload = run_svdpp_experiment(
            ...
            command=(
                "development benchmark svdpp inference materialization training provenance "
                f"--fold {fold_index} --model-seed {model_seed} --epochs {reduced_epochs}"
            ),
        )
        ...
        with runtime_execution_context(threading_config=threading_config):
            ...
            if model.training_backend_effective != "numba":
                raise RuntimeError("svdpp inference benchmark requires effective numba training backend")
            user_factors = model.user_factors
            if user_factors is None:
                raise RuntimeError("trained SVDpp model has no user factors")

            official_users = split.test.user_ids.astype(np.int64, copy=False)
            official_items = split.test.item_ids.astype(np.int64, copy=False)
            ...
            scenarios = [
                ("official_test", official_users, official_items),
            ...
            def reference_predict_many(
                ...
                return _predict_many_full_context_reference(
                    model,
                    user_ids,
                    item_ids,
                    clip=clip,
            ...
            for scenario_name, query_users, query_items in scenarios:
                ...
                if optimized_user_context_cache is None:
                    raise RuntimeError("optimized SVDpp user context cache was not materialized")
                ...
                summary_payload["scenarios"][scenario_name] = {
                    ...
                    "materialization": {
                        "reference_context_rows_per_call": int(user_factors.shape[0]),
                        "optimized_persistent_cache_rows": int(optimized_user_context_cache.shape[0]),
                        "optimized_context_rows_materialized_per_call": 0,
                    },
                    "variants": {
                        "reference_full_context": summarize_inference_variant(query_rows, reference_timings),
        ...
        markdown_lines = [
            "# SVDpp Inference Materialization Compare",
            "",
        ...
        for scenario_name, scenario_payload in summary_payload["scenarios"].items():
            ...
            reference = scenario_payload["variants"]["reference_full_context"]["aggregate"]
            ...
            markdown_lines.extend(
                [
                    ...
                    f"- reference_context_rows_per_call: `{materialization['reference_context_rows_per_call']}`",
                    f"- optimized_persistent_cache_rows: `{materialization['optimized_persistent_cache_rows']}`",
                    (
                    ...
                    "| --- | ---: | ---: | ---: |",
                    (
                        f"| `reference_full_context` | {reference_time['mean']:.6f} | "
