    ...
    *,
    ...
    for user_id in np.unique(users):
        ...
        contexts[int(user_id)] += (
            norm * np.sum(model.implicit_factors[history], axis=0).astype(np.float64, copy=False)
        )
    ...
    try:
        ...
        with runtime_execution_context(threading_config=threading_config):
            ...
            for scenario_name, query_users, query_items in scenarios:
                reference_predict_many = lambda user_ids, item_ids, clip=False: _predict_many_full_context_reference(
                    model,
                    user_ids,
                    item_ids,
                    clip=clip,
                ...
                summary_payload["scenarios"][scenario_name] = {
                    ...
                    "materialization": {
                        "reference_context_rows_per_call": int(model.user_factors.shape[0]),
        ...
        for scenario_name, scenario_payload in summary_payload["scenarios"].items():
            ...
            markdown_lines.extend(
                [
                    ...
                    f"- reference_context_rows_per_call: `{scenario_payload['materialization']['reference_context_rows_per_call']}`",
                    ...
                    "| --- | ---: | ---: | ---: |",
                    f"| `reference_full_context` | {reference['inference_wall_clock_seconds']['mean']:.6f} | {reference['inference_wall_clock_seconds']['coefficient_of_variation']:.6f} | {reference['ratings_per_second_inference']['mean']:.6f} |",
